 ...
 *      "http://www.ngs.noaa.gov/PUBS_LIB/inverse.pdf">Vincenty's original
 *      publication</a> on the NOAA website.
 * 
...
public class GeodeticCalculator {
	 ...
	 * distance, and a specified starting bearing, for an initial location. This is
	 * the solution to the direct geodetic problem.
	 * 
	 ...
	 */
	public GlobalCoordinates calculateEndingGlobalCoordinates(Ellipsoid ellipsoid, GlobalCoordinates start,
		...
		final double phi1 = Angle.toRadians(start.getLatitude());
		final double alpha1 = Angle.toRadians(startBearing);
		...
		final double cosU1 = 1.0 / Math.sqrt(1.0 + tanU1 * tanU1);
		...
		// eq. 8
		final double phi2 = Math.atan2(sinU1 * cosSigma + cosU1 * sinSigma * cosAlpha1,
		...
		// eq. 12
		final double alpha2 = Math.atan2(sinAlpha, -sinU1 * sinSigma + cosU1 * cosSigma * cosAlpha1);

		// build result
		final double latitude = Angle.toDegrees(phi2);
		final double longitude = start.getLongitude() + Angle.toDegrees(L);
	 ...
	 * specified starting bearing, for an initial location. This is the solution to
	...
	public GeodeticCurve calculateGeodeticCurve(Ellipsoid ellipsoid, GlobalCoordinates start, GlobalCoordinates end) {
		...
		final double phi1 = Angle.toRadians(start.getLatitude());
		final double lambda1 = Angle.toRadians(start.getLongitude());
		final double phi2 = Angle.toRadians(end.getLatitude());
		final double lambda2 = Angle.toRadians(end.getLongitude());
		...
		final double U1 = Math.atan(tanU1);
		final double sinU1 = Math.sin(U1);
		final double cosU1 = Math.cos(U1);

		final double tanphi2 = Math.tan(phi2);
		final double tanU2 = (1.0 - f) * tanphi2;
		final double U2 = Math.atan(tanU2);
		...
		for (int i = 0; i < 20; i++) {
			...
			final double sinsigma = Math.sqrt(sin2sigma);
			...
			final double alpha = Math.asin(sinalpha);
			...
			final double change = Math.abs((lambda - lambda0) / lambda);
	 ...
	 * or contracting the reference ellipsoid such that the new ellipsoid passes
	 * through the average elevation of the two positions. A geodetic curve across
	 * the new ellipsoid is calculated. The point-to-point distance is calculated as
	 * the hypotenuse of a right triangle where the length of one side is the
	 * ellipsoidal distance and the other is the difference in elevation.
	 * </p>
	 * 
	 ...
	 * @return solution to the inverse geodetic problem
	 */
	public GeodeticMeasurement calculateGeodeticMeasurement(Ellipsoid refEllipsoid, GlobalPosition start,
			GlobalPosition end) {
		// calculate elevation differences
		final double elev1 = start.getElevation();
		final double elev2 = end.getElevation();
		final double elev12 = (elev1 + elev2) / 2.0;

		// calculate latitude differences
		final double phi1 = Angle.toRadians(start.getLatitude());
		final double phi2 = Angle.toRadians(end.getLatitude());
		final double phi12 = (phi1 + phi2) / 2.0;

		// calculate a new ellipsoid to accommodate average elevation
		final double refA = refEllipsoid.getSemiMajorAxis();
		final double f = refEllipsoid.getFlattening();
		final double a = refA + elev12 * (1.0 + f * Math.sin(phi12));
		final Ellipsoid ellipsoid = Ellipsoid.fromAAndF(a, f);

		// calculate the curve at the average elevation
		final GeodeticCurve averageCurve = calculateGeodeticCurve(ellipsoid, start, end);
