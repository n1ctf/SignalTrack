...
public class SectorSelector extends WWObjectImpl implements SelectListener, MouseListener, MouseMotionListener, RenderingListener {
	...
	protected SectorSelector(WorldWindow worldWindow, RegionShape shape, RenderableLayer rLayer) {
		...
		if (rLayer == null) {
			final String msg = Logging.getMessage("nullValue.Layer");
			Logging.logger().log(java.util.logging.Level.SEVERE, msg);
			throw new IllegalArgumentException(msg);
		}
	...
	public void enable() {
		...
		final LayerList layers = this.getWwd().getModel().getLayers();
...
//
	...
	public void selected(SelectEvent event) {
		if (event == null) {
			final String msg = Logging.getMessage("nullValue.EventIsNull");
		...
		} else if (event.getEventAction().equals(SelectEvent.DRAG) && event instanceof DragSelectEvent dragEvent) {
			final Object topObject = dragEvent.getTopObject();
			if (topObject == null) {
				return;
			}

			final RegionShape dragObject = this.getShape();

			if (this.getOperation() == SIZING) {
				final Sector newSector = this.resizeShape(dragObject, this.getSide());
				if (newSector != null) {
	...
	protected int determineAdjustmentSide(Movable dragObject, double factor) {
		if (dragObject instanceof SurfaceSector quad) {
			final Sector s = quad.getSector();
			final Position p = this.getWwd().getCurrentPosition();

			if (p == null) {
				return NONE;
			}

			final double dN = abs(s.getMaxLatitude().subtract(p.getLatitude()).degrees);
			final double dS = abs(s.getMinLatitude().subtract(p.getLatitude()).degrees);
			final double dW = abs(s.getMinLongitude().subtract(p.getLongitude()).degrees);
			final double dE = abs(s.getMaxLongitude().subtract(p.getLongitude()).degrees);

			final double sLat = factor * s.getDeltaLatDegrees();
			final double sLon = factor * s.getDeltaLonDegrees();
	...
	protected Sector resizeShape(Movable dragObject, int side) {
		if (dragObject instanceof SurfaceSector quad) {
			...
			final Angle dLat = p.getLatitude().subtract(this.getPreviousPosition().getLatitude());
			final Angle dLon = p.getLongitude().subtract(this.getPreviousPosition().getLongitude());
	...
	protected void dragWholeShape(DragSelectEvent dragEvent, Movable dragObject) {
		final View view = getWwd().getView();
		final EllipsoidalGlobe globe = (EllipsoidalGlobe) getWwd().getModel().getGlobe();

		// Compute ref-point position in screen coordinates.
		final Position refPos = dragObject.getReferencePosition();
		if (refPos == null) {
			return;
		}

		final Vec4 refPoint = globe.computePointFromPosition(refPos);
		final Vec4 screenRefPoint = view.project(refPoint);

		// Compute screen-coord delta since last event.
		final int dx = dragEvent.getPickPoint().x - dragEvent.getPreviousPickPoint().x;
		final int dy = dragEvent.getPickPoint().y - dragEvent.getPreviousPickPoint().y;

		// Find intersection of screen coord ref-point with globe.
		final double x = screenRefPoint.x + dx;
		final double y = dragEvent.getMouseEvent().getComponent().getSize().height - screenRefPoint.y + dy - 1;
		final Line ray = view.computeRayFromScreenPoint(x, y);
		final Intersection[] inters = globe.intersect(ray, refPos.getElevation());

		if (inters != null) {
			// Intersection with globe. Move reference point to the intersection point.
			final Position p = globe.computePositionFromPoint(inters[0].getIntersectionPoint());
			dragObject.moveTo(p);
		}
	}
	...
	protected void setCursor(int sideName) {
		...
		switch (sideName) {
			case NONE -> cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR);
			...
			case NORTHWEST -> cursor = Cursor.getPredefinedCursor(Cursor.NW_RESIZE_CURSOR);
			case NORTHEAST -> cursor = Cursor.getPredefinedCursor(Cursor.NE_RESIZE_CURSOR);
			case SOUTHWEST -> cursor = Cursor.getPredefinedCursor(Cursor.SW_RESIZE_CURSOR);
			case SOUTHEAST -> cursor = Cursor.getPredefinedCursor(Cursor.SE_RESIZE_CURSOR);
		}
	...
	protected static class RegionShape extends SurfaceSector {
		...
		protected RegionShape(Sector sector) {
			...
			// Setup default interior rendering attributes. Note that the interior rendering
			// attributes are
			// configured so only the SurfaceSector's interior is rendered.
			final ShapeAttributes interiorAttrs = new BasicShapeAttributes();
			...
			// Setup default border rendering attributes. Note that the border rendering
			// attributes are configured
			// so that only the SurfaceSector's outline is rendered.
			final ShapeAttributes borderAttrs = new BasicShapeAttributes();
		...
		public void setInteriorColor(Color color) {
			final ShapeAttributes attr = this.getAttributes();
		...
		public void setBorderColor(Color color) {
			final ShapeAttributes attr = this.getBorder().getAttributes();
		...
		protected void setBorder(SurfaceSector shape) {
			if (shape == null) {
				final String message = Logging.getMessage("nullValue.Shape");
		...
		public void preRender(DrawContext dc) {
			// This is called twice: once during normal rendering, then again during ordered
			// surface rendering. During
			// normal renering we pre-render both the interior and border shapes. During
			// ordered surface rendering, both
			// shapes are already added to the DrawContext and both will be individually
			// processed. Therefore we just
		...
		public void render(DrawContext dc) {
			...
			// This is called twice: once during normal rendering, then again during ordered
			// surface rendering. During
			// normal rendering we render both the interior and border shapes. During ordered
			// surface rendering, both
			// shapes are already added to the DrawContext and both will be individually
			// processed. Therefore we just
			// call our superclass behavior
			if (dc.isOrderedRenderingMode()) {
			...
			final PickedObjectList pos = dc.getPickedObjects();
			final PickedObject terrainObject = pos != null ? pos.getTerrainObject() : null;
			...
			if (this.getStartPosition() != null) {
				final Position end = terrainObject.getPosition();
