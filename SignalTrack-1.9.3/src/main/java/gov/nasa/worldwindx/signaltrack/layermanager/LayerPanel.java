...
public class LayerPanel extends JPanel {
	...
	protected transient Layer layer; // the layer represented by this instance
	...
	public LayerPanel(final WorldWindow wwd, final Layer layer) {
		...
		final SelectLayerAction action = new SelectLayerAction(wwd, layer, layer.isEnabled());
		...
		this.upButton.addActionListener(_ -> moveLayer(wwd, layer, -1));

		this.downButton = new JButton(DOWN_ARROW);
		this.downButton.addActionListener(_ -> moveLayer(wwd, layer, +1));
		...
		final int index = this.findLayerPosition(wwd, layer);
	...
	protected void moveLayer(WorldWindow wwd, Layer layer, int direction) {
		// Moves the layer associated with this instance in the direction indicated
	...
	protected static class SelectLayerAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			layer.setEnabled(((JCheckBox) actionEvent.getSource()).isSelected());
