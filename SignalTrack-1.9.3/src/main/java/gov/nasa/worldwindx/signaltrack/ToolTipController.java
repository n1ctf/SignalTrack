...
/**
 * Controls display of tool tips on picked objects. Any shape implementing
 * {@link AVList} can participate. Shapes provide tool tip text in their AVList
 * for either or both of hover and rollover events. The keys associated with the
...
public class ToolTipController implements SelectListener, Disposable {
	...
	/**
	 * Create a controller for a specified {@link WorldWindow} that displays tool
	 * tips on hover and/or rollover.
	 *
	 * @param wwd         the World Window to monitor.
	 * @param rolloverKey the key to use when looking up tool tip text from the
	 *                    shape's AVList when a rollover event occurs. May be null,
	 *                    in which case a tool tip is not displayed for rollover
	 *                    events.
	 * @param hoverKey    the key to use when looking up tool tip text from the
	 *                    shape's AVList when a hover event occurs. May be null, in
	 *                    which case a tool tip is not displayed for hover events.
	 */
	...
	/**
	 * Create a controller for a specified {@link WorldWindow} that displays
	 * "DISPLAY_NAME" on rollover.
	 *
	 * @param wwd the World Window to monitor.
	 */
	...
	protected String getHoverText(SelectEvent event) {
		return event.getTopObject() instanceof AVList avList? avList.getStringValue(this.hoverKey) : null;
	}

	protected String getRolloverText(SelectEvent event) {
		return event.getTopObject() instanceof AVList avList ? avList.getStringValue(this.rolloverKey)
				: null;
	}
