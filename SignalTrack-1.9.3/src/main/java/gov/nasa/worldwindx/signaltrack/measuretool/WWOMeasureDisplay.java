...
public class WWOMeasureDisplay implements WWOMeasureTool.MeasureDisplay {
	...
	public String getLabel(String labelName) {
		if (labelName == null) {
			final String msg = Logging.getMessage("nullValue.LabelName");
			Logging.logger().severe(msg);
			throw new IllegalArgumentException(msg);
		}

		final String label = this.avList.getStringValue(labelName);
	...
	public void updateMeasureDisplay(Position position) {
		...
		final String displayString = this.getDisplayString(position, this.measureTool);
		...
		final Vec4 screenPoint = this.computeAnnotationPosition(position, this.measureTool);
		if (screenPoint != null) {
	...
	protected String getDisplayString(Position pos, WWOMeasureTool mt) {
		...
		final String shapeType = this.measureTool.getMeasureShapeType();
		final Rectangle2D.Double shapeRectangle = this.measureTool.getShapeRectangle();
	...
	protected Vec4 computeAnnotationPosition(Position pos, WWOMeasureTool mt) {
		Vec4 surfacePoint = mt.getWwd().getSceneController().getTerrain().getSurfacePoint(pos.getLatitude(),
				pos.getLongitude());
		if (surfacePoint == null) {
			final Globe globe = mt.getWwd().getModel().getGlobe();
	...
	protected String formatLineMeasurements(Position pos, WWOMeasureTool mt) {
		...
		final Double accumLength = this.computeAccumulatedLength(pos, mt);
	...
	protected void formatControlPoints(Position pos, WWOMeasureTool mt, StringBuilder sb) {
		if (mt.getCenterPosition() != null && areLocationsRedundant(mt.getCenterPosition(), pos, mt.getUnitsFormat())) {
			sb.append(mt.getUnitsFormat().angleNL(mt.getLabel(CENTER_LATITUDE_LABEL),
					mt.getCenterPosition().getLatitude()));
	...
	protected Double computeAccumulatedLength(LatLon pos, WWOMeasureTool mt) {
		...
		final double radius = mt.getWwd().getModel().getGlobe().getRadius();
		double distanceFromStart = 0;
		final int segmentIndex = 0;
		LatLon pos1 = mt.getPositions().get(segmentIndex);
		for (int i = 1; i < mt.getPositions().size(); i++) {
			final LatLon pos2 = mt.getPositions().get(i);
			final double segmentLength = LatLon.greatCircleDistance(pos1, pos2).radians * radius;

			// Check whether the position is inside the segment
			final double length1 = LatLon.greatCircleDistance(pos1, pos).radians * radius;
			final double length2 = LatLon.greatCircleDistance(pos2, pos).radians * radius;
		...
		final double gcPathLength = this.computePathLength(mt);
	...
	protected Angle computeAngleBetween(LatLon a, LatLon b, LatLon c) {
		final Vec4 v0 = new Vec4(b.getLatitude().radians - a.getLatitude().radians,
				b.getLongitude().radians - a.getLongitude().radians, 0);

		final Vec4 v1 = new Vec4(c.getLatitude().radians - b.getLatitude().radians,
	...
	protected boolean areLocationsRedundant(LatLon locA, LatLon locB, UnitsFormat units) {
		...
		final String aLat = units.angleNL("", locA.getLatitude());
		final String bLat = units.angleNL("", locB.getLatitude());

		if (!aLat.equals(bLat)) {
			return false;
		}

		final String aLon = units.angleNL("", locA.getLongitude());
		final String bLon = units.angleNL("", locB.getLongitude());
