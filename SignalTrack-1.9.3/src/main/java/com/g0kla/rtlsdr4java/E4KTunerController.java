...
public class E4KTunerController extends RTL2832TunerController {
	...
	public E4KTunerController(Device device, DeviceDescriptor deviceDescriptor) throws DeviceException {
	...
	/*
	 * @Override public void apply( TunerConfiguration config ) throws
	 * DeviceException { if( config != null && config instanceof
	 * E4KTunerConfiguration ) { E4KTunerConfiguration e4kConfig =
	 * (E4KTunerConfiguration)config;
	 * 
	 * try { SampleRate sampleRate = e4kConfig.getSampleRate(); setSampleRate(
	 * sampleRate );
	 * 
	 * double correction = e4kConfig.getFrequencyCorrection();
	 * setFrequencyCorrection( correction );
	 * 
	 * E4KGain masterGain = e4kConfig.getMasterGain(); setGain( masterGain, true );
	 * 
	 * if( masterGain == E4KGain.MANUAL ) { E4KMixerGain mixerGain =
	 * e4kConfig.getMixerGain(); setMixerGain( mixerGain, true );
	 * 
	 * E4KLNAGain lnaGain = e4kConfig.getLNAGain(); setLNAGain( lnaGain, true );
	 * 
	 * E4KEnhanceGain enhanceGain = e4kConfig.getEnhanceGain(); setEnhanceGain(
	 * enhanceGain, true ); }
	 * 
	 * try { setFrequency( e4kConfig.getFrequency() ); } catch( DeviceException se )
	 ...
	 * 
	 * } else { throw new IllegalArgumentException( "E4KTunerController - cannot " +
	 * "apply tuner configuration of type:[" + config.getClass() + "]" ); }
	 * 
	 * }
	 */
	public void init() throws DeviceException {
		...
		final int result = LibUsb.open(mDevice, mDeviceHandle);
		...
		try {
			...
			writeRegister(mDeviceHandle, Block.USB, Address.USB_SYSCTL.getAddress(), (short) 0x09, 1);
	...
	/**
	 * Sets the IF filters (mixer, channel and RC) to the correct filter setting for
	 * the selected bandwidth/sample rate
	 */
	@Override
	public void setSampleRateFilters(int bandwidth) throws DeviceException {
		...
		final boolean i2CRepeaterEnabled = isI2CRepeaterEnabled();

		if (!i2CRepeaterEnabled) {
			enableI2CRepeater(mDeviceHandle, true);
		}

		final boolean controlI2CRepeater = false;

		final MixerFilter mixer = MixerFilter.getFilter(bandwidth);
		setMixerFilter(mixer, controlI2CRepeater);

		final ChannelFilter channel = ChannelFilter.getFilter(bandwidth);
		setChannelFilter(channel, controlI2CRepeater);

		final RCFilter rc = RCFilter.getFilter(bandwidth);
	...
	public long getTunedFrequency() throws DeviceException {
		try {
			...
			final byte z = (byte) readE4KRegister(Register.SYNTH3, controlI2CRepeater);

			final int xHigh = readE4KRegister(Register.SYNTH4, controlI2CRepeater);
			final int xLow = readE4KRegister(Register.SYNTH5, controlI2CRepeater);

			final int x = (Integer.rotateLeft(xHigh, 8)) | xLow;

			final int pllSetting = readE4KRegister(Register.SYNTH7, controlI2CRepeater);
			final PLL pll = PLL.fromSetting(pllSetting);
			...
			throw new DeviceException("E4K tuner controller - couldn't get " + "tuned frequency:" + e);
		}
	}

	public void setTunedFrequency(long frequency) throws DeviceException {
		/* Get the phase locked loop setting */
		final PLL pll = PLL.fromFrequency(frequency);

		/*
		 * Z is an integer representing the number of scaled oscillator frequency
		 * increments the multiplied frequency.
		 */
		final byte z = (byte) (frequency / pll.getScaledOscillator());

		/*
		 * remainder is just as it describes. It is what is left over after we carve out
		 * scaled oscillator frequency increments (z) from the desired frequency.
		 */
		final int remainder = (int) (frequency - ((z & 0xFF) * pll.getScaledOscillator()));
		...
		/**
		 * Hack: if we're trying to set the minimum frequency for the E4K, due to
		 * rounding errors, 52 mhz becomes 51.999993, so we need to adjust x to get
		 * 52.000003 mhz ... otherwise the PLL won't lock on the freq
		 */
		...
		try {
			...
			/* Check for PLL lock */
			final int lock = readE4KRegister(Register.SYNTH1, controlI2CRepeater);

			if ((lock & 0x1) != 0x1) {
				throw new DeviceException("E4K tuner controller - couldn't " + "achieve PLL lock for frequency ["
	...
	private long calculateActualFrequency(PLL pll, byte z, int x) {
		final long whole = pll.getScaledOscillator() * (long) (z & 0xFF);

		final int fractional = (int) (pll.getScaledOscillator() * ((double) x / (double) E4K_PLL_Y));
	...
	public void initTuner(boolean controlI2CRepeater) throws UsbException {
		...
		final boolean i2CRepeaterControl = false;
		...
		writeE4KRegister(Register.MASTER1, (byte) (MASTER1_RESET | MASTER1_NORM_STBY | MASTER1_POR_DET),
				i2CRepeaterControl);
...
//		setDCRangeDetectorEnabled( false, i2CRepeaterControl );
	...
	private void generateDCOffsetTables(boolean controlI2CRepeater) throws UsbException {
		...
		final IFStage1Gain stage1 = getIFStage1Gain(i2CRepeaterControl);
		final IFStage2Gain stage2 = getIFStage2Gain(i2CRepeaterControl);
		final IFStage3Gain stage3 = getIFStage3Gain(i2CRepeaterControl);
		final IFStage4Gain stage4 = getIFStage4Gain(i2CRepeaterControl);
		final IFStage5Gain stage5 = getIFStage5Gain(i2CRepeaterControl);
		final IFStage6Gain stage6 = getIFStage6Gain(i2CRepeaterControl);

		/* Set Mixer gain to manual using current auto Mixer gain setting */
		final E4KMixerGain mixerGain = getMixerGain(i2CRepeaterControl);
		setMixerGain(mixerGain, i2CRepeaterControl);

		/* Set LNA gain to manual using current auto LNA gain setting */
		final E4KLNAGain lnaGain = getLNAGain(i2CRepeaterControl);
		setLNAGain(lnaGain, i2CRepeaterControl);
		...
		/**
		 * Iterate all DC gain combinations, apply gain settings, read the DC offset
		 * value and apply it to the lookup table entry
		 */
		for (DCGainCombination combo : DCGainCombination.values()) {
			...
			final byte offsetI = (byte) (readE4KRegister(Register.DC2, i2CRepeaterControl) & 0x3F);
			final byte offsetQ = (byte) (readE4KRegister(Register.DC3, i2CRepeaterControl) & 0x3F);
			final byte range = (byte) readE4KRegister(Register.DC4, i2CRepeaterControl);
			final byte rangeI = (byte) (range & 0x3);
			final byte rangeQ = (byte) ((range >> 4) & 0x3);

			/* Write the offset and range values to the lookup table */
			writeE4KRegister(combo.getIRegister(), (byte) (offsetI | (rangeI << 6)), i2CRepeaterControl);

			writeE4KRegister(combo.getQRegister(), (byte) (offsetQ | (rangeQ << 6)), i2CRepeaterControl);
		}
	...
	public void setDCRangeDetectorEnabled(boolean enabled, boolean controlI2CRepeater) throws UsbException {
		if (enabled) {
			writeMaskedE4KRegister(Register.DC5, DC5_RANGE_DETECTOR_ENABLED_MASK, DC5_RANGE_DETECTOR_ENABLED,
					controlI2CRepeater);
		} else {
			writeMaskedE4KRegister(Register.DC5, DC5_RANGE_DETECTOR_DISABLED_MASK, DC5_RANGE_DETECTOR_DISABLED,
					controlI2CRepeater);
		}
	}
	...
	public void setEnhanceGain(E4KEnhanceGain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(E4KEnhanceGain.getRegister(), E4KEnhanceGain.getMask(), gain.getValue(),
				controlI2CRepeater);
	}
	...
	public void setMixerGain(E4KMixerGain gain, boolean controlI2CRepeater) throws UsbException {
		...
		final boolean localI2CRepeaterControl = false;

		if (gain == E4KMixerGain.AUTOMATIC) {
			writeMaskedE4KRegister(Register.AGC7, AGC7_MIXER_GAIN_MASK, AGC7_MIX_GAIN_AUTO, localI2CRepeaterControl);
		} else {
			writeMaskedE4KRegister(Register.AGC7, AGC7_MIXER_GAIN_MASK, AGC7_MIX_GAIN_MANUAL, localI2CRepeaterControl);

			/* Set the desired manual gain setting */
			writeMaskedE4KRegister(E4KMixerGain.getRegister(), E4KMixerGain.getMask(), gain.getValue(),
	...
	public E4KMixerGain getMixerGain(boolean controlI2CRepeater) throws UsbException {
		final byte autoOrManual = readMaskedE4KRegister(Register.AGC7, AGC7_MIXER_GAIN_MASK, controlI2CRepeater);

		if (autoOrManual == AGC7_MIX_GAIN_AUTO) {
			return E4KMixerGain.AUTOMATIC;
		} else {
			final int register = readE4KRegister(E4KMixerGain.getRegister(), controlI2CRepeater);
	...
	public void setIFStage1Gain(IFStage1Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage1Gain.getRegister(), IFStage1Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage1Gain getIFStage1Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage1Gain.fromRegisterValue(readE4KRegister(IFStage1Gain.getRegister(), controlI2CRepeater));
	}

	public void setIFStage2Gain(IFStage2Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage2Gain.getRegister(), IFStage2Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage2Gain getIFStage2Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage2Gain.fromRegisterValue(readE4KRegister(IFStage2Gain.getRegister(), controlI2CRepeater));
	}

	public void setIFStage3Gain(IFStage3Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage3Gain.getRegister(), IFStage3Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage3Gain getIFStage3Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage3Gain.fromRegisterValue(readE4KRegister(IFStage3Gain.getRegister(), controlI2CRepeater));
	}

	public void setIFStage4Gain(IFStage4Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage4Gain.getRegister(), IFStage4Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage4Gain getIFStage4Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage4Gain.fromRegisterValue(readE4KRegister(IFStage4Gain.getRegister(), controlI2CRepeater));
	}

	public void setIFStage5Gain(IFStage5Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage5Gain.getRegister(), IFStage5Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage5Gain getIFStage5Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage5Gain.fromRegisterValue(readE4KRegister(IFStage5Gain.getRegister(), controlI2CRepeater));
	}

	public void setIFStage6Gain(IFStage6Gain gain, boolean controlI2CRepeater) throws UsbException {
		writeMaskedE4KRegister(IFStage6Gain.getRegister(), IFStage6Gain.getMask(), gain.getValue(), controlI2CRepeater);
	}

	public IFStage6Gain getIFStage6Gain(boolean controlI2CRepeater) throws UsbException {
		return IFStage6Gain.fromRegisterValue(readE4KRegister(IFStage6Gain.getRegister(), controlI2CRepeater));
	}
	...
	public MixerFilter getMixerFilter(boolean controlI2CRepeater) throws UsbException {
		final int value = readE4KRegister(MixerFilter.getRegister(), controlI2CRepeater);
	...
	public RCFilter getRCFilter(boolean controlI2CRepeater) throws UsbException {
		final int value = readE4KRegister(RCFilter.getRegister(), controlI2CRepeater);
	...
	public void setChannelFilter(ChannelFilter filter, boolean controlI2CRepeater) throws LibUsbException {
		writeMaskedE4KRegister(ChannelFilter.getRegister(), ChannelFilter.getMask(), filter.getValue(),
				controlI2CRepeater);
	}

	public ChannelFilter getChannelFilter(boolean controlI2CRepeater) throws UsbException {
		final int value = readE4KRegister(ChannelFilter.getRegister(), controlI2CRepeater);
	...
	public void setBand(long frequency, boolean controlI2CRepeater) throws UsbException {
		...
		final Band band = Band.fromFrequency(frequency);
		...
		/**
		 * Workaround - reset register (set value to 0) before writing to it, to avoid a
		 * gap around 325-350 MHz
		 */
	...
	private void setRFFilter(long frequency, boolean controlI2CRepeater) throws UsbException {
		final RFFilter filter = RFFilter.fromFrequency(frequency);
	 ...
	 * 
	 * Note: does not respond to the MANUAL setting. Use the manual setting to
	 * configure gui components to expose the individual Mixer, LNA and Enhance gain
	 * settings, to allow the user to change those settings individually.
	 * 
	...
	private byte readMaskedE4KRegister(Register register, byte mask, boolean controlI2CRepeater) throws UsbException {
		final int temp = readE4KRegister(register, controlI2CRepeater);
	...
	private int readE4KRegister(Register register, boolean controlI2CRepeater) throws LibUsbException {
		return readI2CRegister(mDeviceHandle, Register.I2C_REGISTER.getValue(), register.getValue(),
				controlI2CRepeater);
	}

	private void writeE4KRegister(Register register, byte value, boolean controlI2CRepeater) throws LibUsbException {
		writeI2CRegister(mDeviceHandle, Register.I2C_REGISTER.getValue(), register.getValue(), value,
				controlI2CRepeater);
	}
	...
	public enum PLL {
		PLL_72M4(0x0F, 72400000, 48, 600000, true, "72.4 MHz"), PLL_81M2(0x0E, 81200000, 40, 720000, true, "81.2 MHz"),
		...
		private PLL(int value, long frequency, int multiplier, int scaledOscillator, boolean requires3Phase,
				String label) {
			mValue = value;
		...
		/*
		 * Returns the PLL setting with the closest frequency that is greater than the
		 * frequency argument
		 */
	...
	/**
	 * RF filters - UHF Band. Each enum entry defines the center frequency for the
	 * filter and the applicable frequency minimums and maximums for the filter.
	 * 
	 * Note: max frequency overlaps with the next minimum frequency, so valid
	 * frequencies should be less than the maximum frequency defined for the filter.
	 */
	public enum RFFilter {
		NO_FILTER(0, 0, 0),

		// UHF band Filters
		M360(0, 350000000, 370000000), M380(1, 370000000, 392500000), M405(2, 392500000, 417500000),
		M425(3, 417500000, 437500000), M450(4, 437500000, 462500000), M475(5, 462500000, 490000000),
		M505(6, 490000000, 522500000), M540(7, 522500000, 557500000), M575(8, 557500000, 595000000),
		M615(9, 595000000, 642500000), M670(10, 642500000, 695000000), M720(11, 695000000, 740000000),
		M760(12, 740000000, 800000000), M840(13, 800000000, 865000000), M890(14, 865000000, 930000000),
		M970(15, 930000000, 1135000000),

		// L band filters
		M1300(0, 1135000000, 1310000000), M1320(1, 1310000000, 1340000000), M1360(2, 1340000000, 1385000000),
		M1410(3, 1385000000, 1427500000), M1445(4, 1427500000, 1452500000), M1460(5, 1452500000, 1475000000),
		M1490(6, 1475000000, 1510000000), M1530(7, 1510000000, 1545000000), M1560(8, 1545000000, 1575000000),
		M1590(9, 1575000000, 1615000000), M1640(10, 1615000000, 1650000000), M1660(11, 1650000000, 1670000000),
		M1680(12, 1670000000, 1690000000), M1700(13, 1690000000, 1710000000), M1720(14, 1710000000, 1735000000),

		/*
		 * Note: max frequency is currently limited by the max integer value and we set
		 * the max frequency for this filter to 2147 MHz
		 */
		...
		/**
		 * Maximum frequency for this filter is less than this value, but does not
		 * include this value.
		 */
		 ...
		 * @throws IllegalArgumentException if the frequency is outside the max value
		 *                                  (2200 MHz)
		 */
		public static RFFilter fromFrequency(long frequency) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController - " + "cannot find RF filter for frequency [" + frequency + "]");
		}
	}

	public enum MixerFilter {
		BW_27M0(0x00, 27000000, 4800000, 28800000, "27.0 MHz"), BW_4M6(0x80, 4600000, 4400000, 4800000, "4.6 MHz"),
		BW_4M2(0x90, 4200000, 4000000, 4400000, "4.2 MHz"), BW_3M8(0xA0, 3800000, 3600000, 4000000, "3.8 MHz"),
		BW_3M4(0xB0, 3400000, 3200000, 3600000, "3.4 MHz"), BW_3M0(0xC0, 3000000, 2850000, 3200000, "3.0 MHz"),
		BW_2M7(0xD0, 2700000, 2500000, 2850000, "2.7 MHz"), BW_2M3(0xE0, 2300000, 2100000, 2500000, "2.3 MHz"),
		...
		public static MixerFilter fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController - " + "unrecognized mixer filter value [" + value + "]");
		}
	}
	...
	public enum RCFilter {
		...
		BW_12M4(0x04, 12400000, 11500000, 13550000, "12.4 MHz"), BW_10M6(0x05, 10600000, 9800000, 11500000, "10.6 MHz"),
		BW_9M0(0x06, 9000000, 8350000, 9800000, "9.0 MHz"), BW_7M7(0x07, 7700000, 7050000, 8350000, "7.7 MHz"),
		BW_6M4(0x08, 6400000, 5805000, 7050000, "6.4 MHz"), BW_5M3(0x09, 5300000, 4850000, 5805000, "5.3 MHz"),
		BW_4M4(0x0A, 4400000, 3900000, 4850000, "4.4 MHz"), BW_3M4(0x0B, 3400000, 3000000, 3900000, "3.4 MHz"),
		BW_2M6(0x0C, 2600000, 2200000, 3000000, "2.6 MHz"), BW_1M8(0x0D, 1800000, 1500000, 2200000, "1.8 MHz"),
		BW_1M2(0x0E, 1200000, 1100000, 1500000, "1.2 MHz"), BW_1M0(0x0F, 1000000, 0, 1100000, "1.0 MHz");
		...
		public static RCFilter fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController - " + "unrecognized rc filter value [" + value + "]");
		}
	}

	public enum ChannelFilter {
		BW_5M50(0x00, 5500000, 5400000, 28800000, "5.50 MHz"), BW_5M30(0x01, 5300000, 5150000, 5400000, "5.30 MHz"),
		BW_5M00(0x02, 5000000, 4900000, 5150000, "5.00 MHz"), BW_4M80(0x03, 4800000, 4700000, 4900000, "4.80 MHz"),
		BW_4M60(0x04, 4600000, 4500000, 4700000, "4.60 MHz"), BW_4M40(0x05, 4400000, 4350000, 4500000, "4.40 MHz"),
		BW_4M30(0x06, 4300000, 4200000, 4350000, "4.30 MHz"), BW_4M10(0x07, 4100000, 4000000, 4200000, "4.10 MHz"),
		BW_3M90(0x08, 3900000, 3850000, 4000000, "3.90 MHz"), BW_3M80(0x09, 3800000, 3750000, 3850000, "3.80 MHz"),
		BW_3M70(0x0A, 3700000, 3650000, 3750000, "3.70 MHz"), BW_3M60(0x0B, 3600000, 3500000, 3650000, "3.60 MHz"),
		BW_3M40(0x0C, 3400000, 3350000, 3500000, "3.40 MHz"), BW_3M30(0x0D, 3300000, 3250000, 3350000, "3.30 MHz"),
		BW_3M20(0x0E, 3200000, 3150000, 3250000, "3.20 MHz"), BW_3M10(0x0F, 3100000, 3050000, 3150000, "3.10 MHz"),
		BW_3M00(0x10, 3000000, 2975000, 3050000, "3.00 MHz"), BW_2M95(0x11, 2950000, 2925000, 2975000, "2.95 MHz"),
		BW_2M90(0x12, 2900000, 2850000, 2925000, "2.90 MHz"), BW_2M80(0x13, 2800000, 2775000, 2850000, "2.80 MHz"),
		BW_2M75(0x14, 2750000, 2750000, 2775000, "2.75 MHz"), BW_2M70(0x15, 2700000, 2650000, 2750000, "2.70 MHz"),
		BW_2M60(0x16, 2600000, 2575000, 2650000, "2.60 MHz"), BW_2M55(0x17, 2550000, 2525000, 2575000, "2.55 MHz"),
		BW_2M50(0x18, 2500000, 2475000, 2525000, "2.50 MHz"), BW_2M45(0x19, 2450000, 2425000, 2475000, "2.45 MHz"),
		BW_2M40(0x1A, 2400000, 2350000, 2425000, "2.40 MHz"), BW_2M30(0x1B, 2300000, 2290000, 2350000, "2.30 MHz"),
		BW_2M28(0x1C, 2280000, 2260000, 2290000, "2.28 MHz"), BW_2M24(0x1D, 2240000, 2220000, 2260000, "2.24 MHz"),
		BW_2M20(0x1E, 2200000, 2175000, 2220000, "2.20 MHz"), BW_2M15(0x1F, 2150000, 0, 2175000, "2.15 MHz");
		...
		public static ChannelFilter fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController - " + "unrecognized channel filter value [" + value + "]");
		}
	}

	public enum E4KLNAGain {
		AUTOMATIC(-1, "Auto"), GAIN_MINUS_50(0, "-5.0 db"), GAIN_MINUS_25(1, "-2.5 db"), GAIN_PLUS_0(4, "0.0 db"),
		GAIN_PLUS_25(5, "2.5 db"), GAIN_PLUS_50(6, "5.0 db"), GAIN_PLUS_75(7, "7.5 db"), GAIN_PLUS_100(8, "10.0 db"),
		GAIN_PLUS_125(9, "12.5 db"), GAIN_PLUS_150(10, "15.0 db"), GAIN_PLUS_175(11, "17.5 db"),
		GAIN_PLUS_200(12, "20.0 db"), GAIN_PLUS_250(13, "25.0 db"), GAIN_PLUS_300(14, "30.0 db");
		...
		public static E4KLNAGain fromRegisterValue(int registerValue) {
			final int value = registerValue & getMask();
	 ...
	 * 
	 * Manual is an override that keeps each of the settings intact and allows the
	 * user to change each of them individually
	 */
	public enum E4KGain {
	...
	public enum E4KEnhanceGain {
		...
		public static E4KEnhanceGain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized Enhance Gain value [" + value + "]");
		}
	}
	...
	public enum E4KMixerGain {
		...
		public static E4KMixerGain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized Mixer Gain value [" + value + "]");
		}
	}

	public enum IFStage1Gain {
		GAIN_MINUS3(0x0, "-3 db"), GAIN_PLUS6(0x1, "6 db");
		...
		public static IFStage1Gain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized IF Gain Stage 1 value [" + value + "]");
		}
	}

	public enum IFStage2Gain {
		GAIN_PLUS0(0x0, "0 db"), GAIN_PLUS3(0x2, "3 db"), GAIN_PLUS6(0x4, "6 db"), GAIN_PLUS9(0x6, "9 db");
	...
	public enum IFStage3Gain {
		...
		public static IFStage3Gain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized IF Gain Stage 3 value [" + value + "]");
		}
	}

	public enum IFStage4Gain {
		GAIN_PLUS0(0x00, "0 db"), GAIN_PLUS1(0x20, "1 db"), GAIN_PLUS2A(0x40, "2 db"), GAIN_PLUS2B(0x60, "2 db");
		...
		public static IFStage4Gain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized IF Gain Stage 4 value [" + value + "]");
		}
	}
	...
	public enum IFStage5Gain {
		...
		public static IFStage5Gain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized IF Gain Stage 5 value [" + value + "]");
		}
	...
	public enum IFStage6Gain {
		...
		public static IFStage6Gain fromRegisterValue(int registerValue) {
			...
			throw new IllegalArgumentException(
					"E4KTunerController" + " - unrecognized IF Gain Stage 6 value [" + value + "]");
		}
	}
	...
	public enum Register {
		DUMMY(0x00), MASTER1(0x00), MASTER2(0x01), MASTER3(0x02), MASTER4(0x03), MASTER5(0x04), CLK_INP(0x05),
		REF_CLK(0x06), SYNTH1(0x07), SYNTH2(0x08), SYNTH3(0x09), SYNTH4(0x0A), SYNTH5(0x0B), SYNTH6(0x0C), SYNTH7(0x0D),
		SYNTH8(0x0E), SYNTH9(0x0F), FILT1(0x10), FILT2(0x11), FILT3(0x12), GAIN1(0x14), GAIN2(0x15), GAIN3(0x16),
		GAIN4(0x17), AGC1(0x1A), AGC2(0x1B), AGC3(0x1C), AGC4(0x1D), AGC5(0x1E), AGC6(0x1F), AGC7(0x20), AGC8(0x21),
		AGC11(0x24), AGC12(0x25), DC1(0x29), DC2(0x2A), DC3(0x2B), DC4(0x2C), DC5(0x2D), DC6(0x2E), DC7(0x2F),
		DC8(0x30), QLUT0(0x50), QLUT1(0x51), QLUT2(0x52), QLUT3(0x53), ILUT0(0x60), ILUT1(0x61), ILUT2(0x62),
		ILUT3(0x63), DCTIME1(0x70), DCTIME2(0x71), DCTIME3(0x72), DCTIME4(0x73), PWM1(0x74), PWM2(0x75), PWM3(0x76),
		PWM4(0x77), BIAS(0x78), CLKOUT_PWDN(0x7A), CHFILT_CALIB(0x7B), I2C_REG_ADDR(0x7D), MAGIC_1(0x7E), MAGIC_2(0x7F),
		MAGIC_3(0x82), MAGIC_4(0x86), MAGIC_5(0x87), MAGIC_6(0x88), MAGIC_7(0x9F), MAGIC_8(0xA0), I2C_REGISTER(0xC8);
