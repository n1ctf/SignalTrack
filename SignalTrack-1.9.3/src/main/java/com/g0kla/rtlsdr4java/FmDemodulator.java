...
public class FmDemodulator {
	...
	public double demodulate(double fftData, double fftData2) {
		...
		final double g = 0.5f; // magic number of 1/2 seems to work best
		final double num = i[1] * ( q[2] - q[0] ) - q[1] * ( i[2] - i[0] );
		//double num = q[2] * i[1]  - i[2] * q[1];
		final double den = (i[1]*i[1] + q[1]*q[1]); 
		
		double deltafreq =  g * (num/den);
