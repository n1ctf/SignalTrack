...
public class R820TTunerController extends RTL2832TunerController {
	...
	protected static final byte[] BIT_REV_LOOKUP_TABLE = { (byte) 0x0, (byte) 0x8, (byte) 0x4, (byte) 0xC, (byte) 0x2,
			(byte) 0xA, (byte) 0x6, (byte) 0xE, (byte) 0x1, (byte) 0x9, (byte) 0x5, (byte) 0xD, (byte) 0x3, (byte) 0xB,
			(byte) 0x7, (byte) 0xF };

	/* R820T I2C address */
	private static final byte mI2CAddress = (byte) 0x34;
	 ...
	 */
	private final int[] mShadowRegister = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x83, 0x32, 0x75, 0xC0, 0x40, 0xD6, 0x6C,
			0xF5, 0x63, 0x75, 0x68, 0x6C, 0x83, 0x80, 0x00, 0x0F, 0x00, 0xC0, 0x30, 0x48, 0xCC, 0x60, 0x00, 0x54, 0xAE,
			0x4A, 0xC0 };

	// private JPanel mEditor;

	public R820TTunerController(Device device, DeviceDescriptor deviceDescriptor) throws DeviceException {
	...
	@Override
	public void setSampleRateFilters(int sampleRate) {
	...
	public void setTunedFrequency(long frequency) throws DeviceException {
		try {
			...
			final boolean controlI2C = false;

			final long offsetFrequency = frequency + R820T_IF_FREQUENCY;
	 ...
	 */
	@Override
	public void setSamplingMode(SampleMode mode) {
		switch (mode) {
			case QUADRATURE -> {
	...
	private void setMux(long frequency, boolean controlI2C) throws UsbException {
		final FrequencyRange range = FrequencyRange.getRangeForFrequency(frequency);
	...
	public void initTuner(boolean controlI2C) throws UsbException {
		...
		// Not sure why this is always called, In the C source this function returns in
		// the tuner type is
		// not TUNER_ANALOG_TV
	...
	private void setTVStandard(boolean controlI2C) throws UsbException {
		...
		final byte filtQ = 0x10;

		writeR820TRegister(Register.FILTER_CALIBRATION_CODE, (byte) (calibrationCode | filtQ), controlI2C);
	...
	private void systemFrequencySelect(long frequency, boolean controlI2C) throws UsbException {
		...
		writeR820TRegister(Register.MIXER_TOP, mixerTop, controlI2C);
		...
		writeR820TRegister(Register.CP_CUR, cpCur, controlI2C);

		writeR820TRegister(Register.DIVIDER_BUFFER_CURRENT, divBufCur, controlI2C);
		...
		writeR820TRegister(Register.MIXER_TOP2, mixerTop, controlI2C);
	...
	private void setPLL(long frequency, boolean controlI2C) throws UsbException {
		...
		final FrequencyDivider divider = FrequencyDivider.fromFrequency(frequency);

		final int statusRegister4 = getStatusRegister(4, controlI2C);

		final int vco_fine_tune = (statusRegister4 & 0x30) >> 4;

		final int div_num = divider.getDividerNumber(vco_fine_tune);

		writeR820TRegister(Register.DIVIDER, (byte) (div_num << 5), controlI2C);

		/* Get the integral number for this divider and frequency */
		final Integral integral = divider.getIntegral(frequency);
		 ...
		 */
		final int sdm = divider.getSDM(integral, frequency);
	...
	private boolean isPLLLocked(boolean controlI2C) throws UsbException {
		final int register = getStatusRegister(2, controlI2C);
	 ...
	 * @throws UsbException
	 */
	private void initializeRegisters(boolean controlI2C) {
	 ...
	 */
	private int getStatusRegister(int register, boolean controlI2C) {
	...
	public void writeR820TRegister(Register register, byte value, boolean controlI2C) throws UsbException {
		if (register.isMasked()) {
			final int current = mShadowRegister[register.getRegister()];
...
//        		String.format( "%02X", value ) );
	...
	public int readR820TRegister(Register register, boolean controlI2C) {
		return readI2CRegister(mDeviceHandle, mI2CAddress, (byte) register.getRegister(), controlI2C);
	}
