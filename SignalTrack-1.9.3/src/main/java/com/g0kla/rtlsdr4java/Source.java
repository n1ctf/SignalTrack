...
public abstract class Source {
	...
	public int read(double[] readBuffer) {
		...
		// We block until we have read readBuffer length bytes, assuming we are still
		// running
		while (running && doublesRead < readBuffer.length) { // 2 bytes for each sample
			if (buffer.size() > 2) {// if we have at least one set of bytes, then read them
				try {
					...
					// If this happens, we are in an unusual situation. We waited until the
					// circularBuffer contains readBuffer.length of data
					// then we started to read it one byte at a time. However, we have moved the
					// read (start) pointer as far as the end
