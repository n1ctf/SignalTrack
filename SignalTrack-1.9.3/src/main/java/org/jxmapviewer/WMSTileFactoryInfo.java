...
public class WMSTileFactoryInfo extends TileFactoryInfo {
	...
	public String getTileUrl(int x, int y, int zoom) {
		final int tileSize = getTileSize(zoom);

		zoom = getTotalMapZoom() - zoom;
		final int z = (int) Math.pow(2, (double) zoom - 1);

		final int m = x - z;
		final int n = z - 1 - y;

		final int tilesPerDimension = (int) Math.pow(2, zoom);

		final double radius = (tileSize * tilesPerDimension) / (2 * Math.PI);
		final double ulx = MercatorUtils.xToLong(m * tileSize, radius);
		final double uly = MercatorUtils.yToLat(n * tileSize, radius);
		double lrx = MercatorUtils.xToLong((m + 1) * tileSize, radius);
		final double lry = MercatorUtils.yToLat((n + 1) * tileSize, radius);

		if (lrx < ulx) {
			lrx = -lrx;
		}
		final String bbox = ulx + "," + uly + "," + lrx + "," + lry;
		return getBaseURL() + "?version=1.1.1&request=GetMap&layers=" + this.getLayers() + "&format="
