...
public class GeoBounds {
	 ...
	 * Constructor.
	 * 
	 * @param geoPositions A non null list of 2 or more different
	 *                     <code>GeoBounds</code> objects.
	 */
	public GeoBounds(Set<GeoPosition> geoPositions) {
		if (geoPositions == null || geoPositions.size() == 0) {
			throw new IllegalArgumentException(
					"The attribute 'geoPositions' cannot be null and must " + "have 1 or more elements.");
		}
	...
	private void setRect(double minLat, double minLng, double maxLat, double maxLng) {
		...
		if (minLng > maxLng) {
			if (minLng > 0 && minLng < 180 && maxLng < 0) {
				...
				throw new IllegalArgumentException(
						"GeoBounds is not valid - minLng must be less than or equal maxLng or "
	...
	public GeoPosition getCenter() {
		final GeoPosition nw = getNorthWest();
		final GeoPosition se = getSouthEast();
		final double lat = (nw.getLatitude() + se.getLatitude()) * 0.5;
		final double lon = (nw.getLongitude() + se.getLongitude()) * 0.5;
