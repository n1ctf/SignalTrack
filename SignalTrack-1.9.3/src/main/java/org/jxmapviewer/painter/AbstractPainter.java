 ...
 * may extend. It extends {@link org.jxmapviewer.beans.Bean} as a convenience
 * for adding property change notification support. In addition,
...
public abstract class AbstractPainter<T> extends AbstractBean implements Painter<T> {
	...
	public final BufferedImageOp[] getFilters() {
		final BufferedImageOp[] results = new BufferedImageOp[filters.length];
	...
	public void setFilters(BufferedImageOp... effects) {
		...
		final BufferedImageOp[] old = getFilters();
	...
	public void setAntialiasing(boolean value) {
		final boolean old = isAntialiasing();
		antialiasing = value;
		if (old != value) {
			setDirty(true);
		}
	...
	public void setInterpolation(Interpolation value) {
		final Object old = getInterpolation();
	...
	public void clearCache() {
		final BufferedImage cache = cachedImage == null ? null : cachedImage.get();
		if (cache != null) {
			cache.flush();
		}
		cacheCleared = true;
	...
	public final void paint(Graphics2D g, T obj, int width, int height) {
		...
		if (shouldUseCache() || filters.length > 0) {
			...
			final boolean invalidCache = null == cache || cache.getWidth() != width || cache.getHeight() != height;
			...
			if (cacheCleared || invalidCache || isDirty()) {
				...
				if (cache != null) {
					final Graphics2D gfx = cache.createGraphics();
					...
					try {
						...
						if (!invalidCache) {
							...
							final Composite composite = gfx.getComposite();
