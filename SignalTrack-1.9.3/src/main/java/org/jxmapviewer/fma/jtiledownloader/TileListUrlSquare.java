...
public class TileListUrlSquare extends TileListCommonBBox {
	...
	public void calculateTileValuesXY() {
		...
		log.log(Level.FINE, "calculate tile values for (UrlSquare:) lat {0}, lon {1}, radius {2}",
				new Object[] { _latitude, _longitude, _radius });

		if (_radius > 6370000 * 2 * 4) {
			_radius = 6370000 * 2 * 4;
		}

		final double minLat = _latitude - 360 * (_radius / Constants.EARTH_CIRC_POLE);
		final double minLon = _longitude
				- 360 * (_radius / (Constants.EARTH_CIRC_EQUATOR * Math.cos(_longitude * Math.PI / 180)));
		final double maxLat = _latitude + 360 * (_radius / Constants.EARTH_CIRC_POLE);
		final double maxLon = _longitude
				+ 360 * (_radius / (Constants.EARTH_CIRC_EQUATOR * Math.cos(_longitude * Math.PI / 180)));
