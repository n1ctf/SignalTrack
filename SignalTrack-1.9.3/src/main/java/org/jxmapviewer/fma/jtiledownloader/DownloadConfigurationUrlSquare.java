...
public class DownloadConfigurationUrlSquare extends DownloadConfiguration {
	private static final Logger LOG = Logger.getLogger(DownloadConfigurationUrlSquare.class.getName());
	...
	public TileList getTileList(DownloadJob downloadJob) {
		final TileListUrlSquare tileList = new TileListUrlSquare();

		final String url = getPasteUrl();
		if (url == null || url.isEmpty()) {
			LOG.log(Level.WARNING, "Invalid URL: {0}", url);
		}

		tileList.setDownloadZoomLevels(
				Util.getOutputZoomLevelArray(downloadJob.getTileProvider(), downloadJob.getOutputZoomLevels()));
